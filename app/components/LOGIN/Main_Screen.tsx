...
import Banner_mobile from "./Banner_mobile";
import Banner_mobile_login from "./Banner_mobile_login";
import Banner_mobile_register from "./Banner_mobile_register";
...
function Main_Screen() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 m-auto w-full  max-w-[1400px]">
      {/* PC VERSION */}
      ...
      <AnimatePresence mode="wait">
        {status === "default" && (
          <motion.div
            ...
            exit={{ opacity: 0, x: -100 }}
          >
            <Banner_mobile onClick={handleClick} />
          </motion.div>
        )}
        {status === "login" && (
          <motion.div
            ...
            exit={{ opacity: 0, x: -100 }}
          >
            <Banner_mobile_login onClick={handleClick} />
          </motion.div>
        )}
        {status === "register" && (
          <motion.div
            ...
            exit={{ opacity: 0, x: -100 }}
          >
            <Banner_mobile_register onClick={handleClick} />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
